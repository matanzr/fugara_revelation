...
USER=pi
FLAGS="-avh --delete --exclude='*.png'"

#--- end config

echo to remove password use: ssh-copy-id pi@rev4.local

rsync $FLAGS $FROM $USER@rev1.local:$TO
rsync $FLAGS $FROM $USER@rev2.local:$TO
rsync $FLAGS $FROM $USER@rev3.local:$TO
rsync $FLAGS $FROM $USER@rev4.local:$TO
rsync $FLAGS $FROM $USER@rev5.local:$TO
rsync $FLAGS $FROM $USER@rev6.local:$TO